...
help:
	@echo 'Management commands for dispute:'
	@echo
	@echo 'Usage:'
...
build-service:
	...
	mkdir -p ./bin
	go build -ldflags "-w -X main.GitCommit=${GIT_COMMIT} -X main.Version=${VERSION} -X main.BuildDate=${BUILD_DATE}" -o ./bin/dispute ./cmd/dispute/
...
deploy-dev: get-credentials-dev docker-build-dev push-dev
	kubectl set image deployment/dispute dispute=gcr.io/gems-org/dispute-dev:$(VERSION)

deploy-prod: get-credentials-prod docker-build-prod push-prod
	kubectl set image deployment/dispute dispute=gcr.io/gems-org/dispute:$(VERSION)
...
build-migrations:
	docker build -t dispute-migration migrations
